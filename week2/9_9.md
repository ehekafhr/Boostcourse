...
* 사실 일반적으로 leaky average를 할 때는 $\gamma x + (1-\gamma) g$ 로 scaling이 있다.
...
RMSProp처럼, Adagrad를 변형한 알고리즘이다.

$s_t$ 의 계산은 같지만, ( $\gamma$ 대신에 $\rho$ 를 쓴다)

$g_t'$ 또한 $s_t$ 처럼 leaky average를 구해 주어 $\eta$ 를 대신한다.

즉, learning rate의 초기값을 정해 줄 필요가 없는 알고리즘이다.

gradient descent에서 빼지는 값 전체를 rescaled gradient $g_t'$ 라고 하고, 이 값의 leaky average $\triangle x_t$ 를 통해 다음 $g_t'$ 를 계산한다.

### 계산

$$ g_t' = \frac{\sqrt{\triangle x_{t-1} - \epsilon}}{\sqrt{s_t + \epsilon}}  $$

where

$$ \triangle x_t = \rho \triangle x_{t-1} + (1-\rho){g_t'}^{2} $$

똑같이, $\epsilon$ 은 단순히 계산 안정성을 위한 값이다.
...
즉, 1차 모멘트(그래디언트)와 2차 모멘트(그래디언트 제곱)의 leaky average를 구하고, scaling하는 작업까지 한다.

1차 모멘트는 이전의 이동을 계산하여 이동 방향을 유지하는 데 쓰이고,

2차 모멘트는 그래디언트 크기를 조절하는 데 쓰인다.

1차 모멘트, 2차 모멘트의 감쇠인자를 각각 $\beta_1$ , \beta_2$ 로 hyperparameter를 받는다.
...
$$ s_t \leftarrow \beta_2 s_{t-1} + (1-\beta_2)g_{t}^{2} $$

이때, $\beta_1$ 과 $\beta_2$ 의 차이에 의해 1차 모멘트와 2차 모멘트의 이동 속도가 차이가 나게 된다.

따라서, 등비수열의 합의 공식의 역수를 취해(등비수열의 합의 분모값인 1- $\beta_1$ 혹은 $\beta_2$ 값은 초기에 정하고 유지하므로 생각하지 않는다)

각각 (1- $\beta_{1}^{t}$ ) , (1- $\beta_{2}^{t}$ )로 나눈 값을 사용한다.

즉, 

$$ \hat{v_t} = \frac{v_t}{1-\beta_{1}^{t}} , \ \hat{s_t} = \frac{s_t}{1-\beta_{2}^{t}} $$

$$ g_t' = \frac{\eta \hat{v_t}}{\sqrt{\hat{s_t}}+\epsilon} $$

이다. 이때, $\eta$ 가 다시 생긴것과 $\epsilon$ 이 root 밖에 있는 것을 볼 수 있다.

RMSProp의 분산 때문에  $\epsilon$ 이 root 밖에 있는 것이 실전에선 좋다고 한다..

# 피어세션

첫번째 과제를 너무 어렵게 생각한 것 같다.. 그냥 np.linalg 내장함수를 쓰는 과제였나보다.

bitnet에 관한 조형석님의 발표가 있었다. 

요약하면, 양자화를 진행하여 학습하는 것이 빠르고, 정확도 손실도 없다는 내용이다. 

경량화와 관련하여 pruning을 통한 sparse matrix 관련 이야기가 나왔다.

sparse matrix를 만들어서 indexing을 통해 더 빠른 계산이 가능한데, 최신 아키텍쳐에서는 메모리 자체를 움직여가면서 연산을 효율적으로 수행하도록 하기 때문에 이러한 기법이 불가능하다는 이야기가 나왔다.

물론, 이러한 고성능의 gpu나 tpu가 아닌 상황에서는 사용 가능하기에, 아키텍쳐가 어떻게 발전하는지를 더 보고 판단할 필요가 있다.
