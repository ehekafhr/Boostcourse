...
일반적으로 lr 조정 없이 SGD를 쓰게 되면, 나중에 들어온 값의 gradient가 더 큰 영향력을 가지게 된다.

이를 방지하기 위해, 이전 값들의 gradient를 더해 주는 것을 Leaky average 방식이라고 한다.

이전 값들을 저장할 정도를 선택하는 hyperparameter $\beta$ 를 정한다(0과 1 사이)

update를 할 때 gradient 대신 사용할 $\mathbf{v}_t = \beta \mathbf{v}_{t-1} + gradient_t$ 를 구해 준다. 

이렇게 하면, 이전 값들이 (점점 줄어들지만) 영향을 계속 끼쳐 조금 Leaky하지만, 각각의 데이터의 gradient 영향력이 비슷해지게 된다.

### 계산

Leaky average로 구한 $\mathbf{v}_t$ 를 gradient 대신 사용하고, 곱해지 learning rate $\eta_t$ 또한 $\frac{1}{t}$ 등 변하는 값으로 넣어 준다.
